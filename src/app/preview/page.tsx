...
import { Mail, Phone, MapPin, Linkedin, Globe, ExternalLink, Download, Sparkles, Link as LinkIcon } from 'lucide-react';
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-500 print:bg-white print:p-0">
      <div className="flex justify-between items-center mb-4 print:hidden">
        <h1 className="text-2xl font-headline font-bold">Portfolio Preview</h1>
        <Button onClick={handlePrint} className="gap-2">
          <Download className="w-4 h-4" /> Save as PDF / Print
        </Button>
      </div>

      <div className="bg-white text-slate-900 shadow-2xl rounded-2xl overflow-hidden border border-slate-200 mx-auto max-w-5xl">
        {/* Header Section */}
        ...
        <div className="p-8 md:p-12 grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          <div className="lg:col-span-2 space-y-12">
            <section className="space-y-4">
              ...
              <p className="text-slate-600 leading-relaxed text-lg italic opacity-80 mb-4">
                {data.about.shortBio}
              </p>
                ...
                {data.experience.map((job) => (
                  <div key={job.id} className="relative pl-8 before:absolute before:left-0 before:top-2 before:w-3 before:h-3 before:bg-primary before:rounded-full before:z-10 after:absolute after:left-[5px] after:top-4 after:bottom-0 after:w-[2px] after:bg-slate-100 last:after:hidden">
                    <div className="flex flex-wrap justify-between items-baseline mb-2">
                      ...
                      <span className="text-sm font-medium text-slate-500 bg-slate-100 px-2 py-1 rounded">{job.startDate} - {job.endDate || 'Present'}</span>
                    </div>
            ...
            <section className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {data.projects.map((proj) => (
                  <Card key={proj.id} className="overflow-hidden border-slate-200 group hover:shadow-lg transition-shadow">
                    <div className="aspect-video relative bg-slate-100 overflow-hidden">
                      {proj.imageUrl ? (
                        <Image src={proj.imageUrl} alt={proj.name} fill className="object-cover group-hover:scale-105 transition-transform duration-500" />
                      ) : (
            ...
            <section className="space-y-4">
              ...
              <div className="flex flex-wrap gap-2">
                {data.about.strengths.map((s, i) => (
                  <Badge key={i} className="bg-slate-100 text-slate-800 hover:bg-primary hover:text-white transition-colors">{s}</Badge>
                ))}
              </div>
            </section>
